...
const { getOfficialPage, selectorsForCurrentUrl } = require('../support/universityPages');
...
Given('I open Google in the browser', async function () {
  ...
  try {
    const acceptBtn = await driver.$('button[id*="accept"], button[aria-label*="Aceptar"]');
...
When('I open the official {string} page for {string}', async function (section, university) {
  let html = await driver.getPageSource();
  if (hasCaptcha(html)) {
    if (!allowManualCaptcha) {
      throw new Error('Google mostró un CAPTCHA por tráfico automático. Ejecuta con ALLOW_MANUAL_CAPTCHA=true para resolverlo manualmente o cambia de red/IP.');
    }
    ...
    await driver.waitUntil(
      ...
      { timeout: 180000, interval: 2000, timeoutMsg: 'CAPTCHA no fue resuelto en 3 minutos.' }
    );
  }
...
Then('the current URL should contain the expected official host', async function () {
  const expected = this.__expectedOfficialHost;
  assert.ok(expected, 'Falta host esperado: ejecuta antes "I open the official ... page".');
...
When('I search for {string} within the page using id-first selectors', async function (searchTerm) {
  const currentUrl = await driver.getUrl();
